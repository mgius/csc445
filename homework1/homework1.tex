...
\intertext{Now I need to prove that the remainder is divisible by 6}
6 & \mid 3n^2 + 3n \\
6 & \mid 3(n^2 + n) \\
2 & \mid n^2 + n \\ 
\intertext{Now I need to prove that $n^2 + n$ is divisible by 2, or even}
\end{align*}
...
\begin{align*}
\intertext{It is a given that:}
\sum_{1}^{n} n & = \frac{(n)(n+1)}{2}
\intertext{The maximum degree of any graph occurs when each vertex is connected
   to every other vertex.  Therefore, the maximum degree for each vertex is 
   $(V - 1)$, and the sum of the max degree'd vertexes is:}
\sum_{1}^{n} n-1 = (n)(n-1) \\ 
\intertext{So, we now know that: }
n(n-1) & \leq 2e \\
\frac{n(n-1)}{2} & \leq e \leq \frac{n(n-1)}{2} \\
\frac{n(n-1)}{2} & \leq \frac{n(n-1)}{2} \\
\end{align*}
\end{enumerate}
\end{document}
